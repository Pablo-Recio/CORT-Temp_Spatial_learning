...
#
#| label: ROS-learning
tidy_post_fig_ROS <- post_gen %>%
  mutate(CORT_Cold = b_ROS_Intercept,
      Control_Cold = b_ROS_Intercept + b_ROS_cortControl,
      CORT_Hot = b_ROS_Intercept + b_ROS_tempHot,
      Control_Hot = b_ROS_Intercept + b_ROS_cortControl + b_ROS_tempHot + `b_ROS_cortControl:tempHot`) %>%
  select(CORT_Cold, Control_Cold, CORT_Hot, Control_Hot) %>%
  pivot_longer(cols = c(CORT_Cold, Control_Cold, CORT_Hot, Control_Hot), names_to = "Treatment", values_to = "ROS") %>%
  mutate(Treatment = factor(Treatment,
    levels = c("CORT_Cold", "Control_Cold", "CORT_Hot", "Control_Hot"),
    labels = c("CORT_Cold" = "CORT-Cold (n=20)",
              "Control_Cold" = "Control-Cold (n=20)",
              "CORT_Hot" = "CORT-Hot (n=18)",
              "Control_Hot" = "Control-Hot (n=20)")
  )) %>%
data.frame()
#
data_plot_ROS <- tidy_post_fig_ROS %>%
  group_by(Treatment) %>%
  summarize(
    Mean = mean(ROS),
    SD = sd(ROS),
    SE = sd(ROS)/sqrt(length(ROS))
  ) %>%
  ungroup() %>%
data.frame()
write.csv(data_plot_ROS, here("output/Checking/data_plot_ROS.csv"))
#
fig_ROS <- ggplot(tidy_post_fig_ROS, aes(x = Treatment, y = ROS, fill = Treatment)) +
  geom_flat_violin(alpha = 0.5) +
  scale_fill_manual(values = c("CORT-Cold (n=20)"="#00008B", "Control-Cold (n=20)"="#68bde1", 
                    "CORT-Hot (n=18)"="#b50101", "Control-Hot (n=20)"="#fa927d")) +
  geom_point(data = data_plot_ROS, aes(y = Mean, x = Treatment), position = position_dodge(width = 0.75), color = "black", fill = "black", size = 3) +
  geom_segment(data = data_plot_ROS, aes(y = Mean - SD, yend = Mean + SD, x = Treatment, xend = Treatment), size = 1.5, color = "black") +
  geom_hline(yintercept = mean(data_plot_ROS$Mean), linetype = "dashed", color = "black") +
  ylim(min(tidy_post_fig_ROS$ROS), max(tidy_post_fig_ROS$ROS)) +
  coord_flip() +
  theme_classic() +
...
ggsave("./output/figures/fig_ROS.png", plot=fig_ROS, width = 15, height = 15, units = "cm", dpi = 600)
#
#
r_choice <- cor.test(clean_df$ROS, clean_df$choice_slope_mean, method = "pearson")
r_choice_text <- paste("r = ", round(r_choice$estimate, 2),
              " [", round(r_choice$conf.int[1], 2), ", ", round(r_choice$conf.int[2], 2), "]",
              sep = "")
#
r_errors <- cor.test(clean_df$ROS, clean_df$errors_slope_mean, method = "pearson")
r_errors_text <- paste("r = ", round(r_errors$estimate, 2),
              " [", round(r_errors$conf.int[1], 2), ", ", round(r_errors$conf.int[2], 2), "]",
              sep = "")
#
plot_ROS_choice <- ggplot(data = clean_df, aes(x = ROS, y = choice_slope_mean)) +
  geom_smooth(method = "lm", color = "#8D0001", fill = "#FFC3C3") +
  theme_classic() +
  labs(y = "Estimated learning slope (Choice)", x = "ROS in medial cortex") +
  theme(plot.margin = margin(5.5, 5.5, 5.5, 5.5, "mm")) +
  theme(
    axis.title = element_text(size = 12, family = "Times"),
    axis.text = element_text(size = 10, family = "Times"),
...
annotate("text", x = Inf, y = Inf, label = r_choice_text, 
           hjust = 1.05, vjust = 1.95, size = 4.5, color = "#8D0001")
#
plot_ROS_errors <- ggplot(data = clean_df, aes(x = ROS, y = errors_slope_mean)) +
  geom_smooth(method = "lm", color = "#8D0001", fill = "#FFC3C3") +
  theme_classic() +
  labs(y = "Estimated learning slope (Errors)", x = "ROS in medial cortex") +
  theme(plot.margin = margin(5.5, 5.5, 5.5, 5.5, "mm")) +
  theme(
    axis.title = element_text(size = 12, family = "Times"),
    axis.text = element_text(size = 10, family = "Times"),
...
annotate("text", x = Inf, y = Inf, label = r_errors_text, 
           hjust = 1.05, vjust = 1.95, size = 4.5, color = "#8D0001")
#
fig_ROS_learning <- plot_grid(plot_ROS_choice, plot_ROS_errors, nrow = 1) 
ggsave("./output/figures/fig_ROS_learning.png", plot=fig_ROS_learning, width = 25, height = 15, units = "cm", dpi = 600)
